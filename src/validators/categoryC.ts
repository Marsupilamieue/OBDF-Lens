...
function findInDdl(
  ...
  const absEndChar =
    endLineOffset === 0 ? view.viewDdlStartChar + endCharOnLine : endCharOnLine;
...
function reportSelectColumnError(
  ...
  const c3Range = findInDdl(
    view,
    ref.display,
    selectIdx !== -1 ? selectIdx : 0,
  );
  const replacement = closest
    ? ref.tableAlias
      ? `${ref.tableAlias}.${closest.match}`
      : closest.match
    : undefined;
...
export async function validateCategoryC(
  ...
  for (const model of vdbData.models) {
    for (const view of model.views) {
      ...
      // C1a: sourceName from DDL has no matching connection config
      if (!connConfig) {
      ...
      try {
        ...
        const causeMsg =
          firstCause instanceof Error
            ...
            : firstCause?.message ||
              (firstCause?.address
                ? `${firstCause.address}:${firstCause.port}`
                : null);
        const errMsg =
          [anyErr?.code, causeMsg].filter(Boolean).join(" — ") ||
      ...
      const tableExists = tables.some(
        (t) => t.toLowerCase() === view.tableName.toLowerCase(),
      );

      if (!tableExists) {
        const closest = findClosest(view.tableName, tables);
        const fromIdx = view.ddl.search(/\bFROM\b/i);
        const c2Range = findInDdl(
          view,
          view.tableName,
          fromIdx !== -1 ? fromIdx : 0,
        );
        ...
        msg += tables
          .map((t) =>
            t === closest?.match ? `  • ${t}   <- paling mirip` : `  • ${t}`,
          )
          .join("\n");
      ...
      const selectRefs = parseSelectColumnRefs(
        extractSelectClauseFromDdl(view.ddl),
      );
      ...
      for (const ref of selectRefs) {
        ...
        const exists = tableColumns.some(
          (c) => c.toLowerCase() === ref.column.toLowerCase(),
        );
        if (!exists) {
      ...
      for (const ref of joinRefs) {
        ...
        if (!tableName) {
          ...
          const data: ObdfDiagnosticData = {
            code: 'C4',
            fixes: closestAlias ? [{
              title: `Ganti alias '${ref.alias}' → '${closestAlias.match}'`,
              edits: [toFixEdit(vdbUri, c4AliasRange, closestAlias.match)],
            }] : [],
          };

          let msg = `[C4] Alias tabel '${ref.alias}' tidak terdefinisi di klausa FROM/JOIN\n`;
          if (closestAlias) {
            msg += `Suggestion: Maksud kamu alias '${closestAlias.match}'? (similarity: ${pct(closestAlias.score)})\n`;
          }
          msg += `Alias yang tersedia: ${availableAliases.join(', ') || '(tidak ada)'}`;

          const diag = new vscode.Diagnostic(c4AliasRange, msg, vscode.DiagnosticSeverity.Error);
          diag.code = 'C4';
          diag.source = 'OBDF Lens';
          (diag as any).data = data;
          diagnostics.push(diag);
